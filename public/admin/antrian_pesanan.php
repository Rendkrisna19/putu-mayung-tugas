...
$sql = "SELECT order_items.id AS order_id, products.nama_product AS product_name, order_items.jumlah, payments.bukti_pembayaran, payments.status 
    FROM order_items 
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<td class='px-4 py-2'>";
                                echo "<form action='selesai_pesanan.php' method='POST'>";
                                echo "<input type='hidden' name='order_id' value='" . $row["order_id"] . "'>";
                                echo "<button type='submit' class='bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700 transition'>Selesai</button>";
                                echo "</form>";
                                echo "</td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='6' class='px-4 py-2 text-center text-gray-500'>No orders found</td></tr>";
                        }
                        $conn->close();
                        ?>
                    </tbody>
